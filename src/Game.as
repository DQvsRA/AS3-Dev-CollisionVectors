package  {
	...
	import classes.entity.BoundingBox;
	...
	public class Game extends Sprite {
		...
		private static var RADIUS_TO_GENERATE: uint = 100;
		...
		private var _forms:Vector.<Form> = new Vector.<Form>();
		...
		public function Game() {
			...
			_graphicsMouse.lineStyle(1, 0xffcc00);
			_graphicsMouse.drawCircle(0, 0, RADIUS_TO_GENERATE);
			_graphicsMouse.endFill();
		...
		private function handle_KeyboardKeyUp(e:KeyboardEvent):void {
			...
			if (e.keyCode == Keyboard.E) {
				...
				while (_counter--) {
					_form = _forms[_counter];
					this.removeChild(_form);
				}
				_forms = new Vector.<Form>();
			}
		}
		...
		private function process_DrawPolygonShape():void {
			...
			var shapeCenterX:Number = (_point.x  + _nextPoint.x) * tempcalc;
			var shapeCenterY:Number = (_point.y  + _nextPoint.y) * tempcalc;
			...
			_shapeCenter = new Point(shapeCenterX * tempcalc, shapeCenterY * tempcalc);
		...
		private function process_Regenerate():void {
			...
			//process_DrawTrianglesShape();
			_counter = _forms.length;
			var radiusToCheck:uint = 0;
			var collisionpoints:Vector.<Point> = new Vector.<Point>();
			var tempPoints:Vector.<Point>;
			var distance:uint = 0;
			var click:Point = _positionLocal.clone();
			while (_counter--) {
				_form 			= _forms[_counter];
				radiusToCheck 	= _form.radius + RADIUS_TO_GENERATE;
				distance 		= Point.distance(_form.position, click);
				if (distance < radiusToCheck) {
					tempPoints = _form.getCollisionPoints(click, RADIUS_TO_GENERATE);
					radiusToCheck = tempPoints.length;
					trace("COLLISTION: " + radiusToCheck);
					if (radiusToCheck > 0) 
						while (radiusToCheck--) 
							collisionpoints.push(tempPoints[radiusToCheck]);
					//collisionpoints = collisionpoints.concat(tempPoints);
				}
			}
			trace(collisionpoints);
			
			var shapeSides	:uint 		= Utils.random(4, 9);
			var shapeRadius	:uint 		= RADIUS_TO_GENERATE;//Utils.random(100, 300);
			var lineColor	:uint 		= Utils.getRandomColor().getOriginalHex();
			var lineWidth	:uint 		= 1;//Utils.random(1, 5);
			var lineAlpha	:Number 	= 0.4;//Utils.random(0.1,1);
			var shapeColor	:uint 		= Utils.getRandomColor().getOriginalHex();
			var shapeAlpha	:Number 	= 0.1;//Utils.random(0.1,1);
			
			var formParams:FormParams = new FormParams(
							shapeSides,
							shapeRadius,
							_positionLocal.clone()
						);
						
			formParams.drawLines = true;	
			formParams.drawRadius = true;
			formParams.drawCenter = true;
			formParams.drawTris = true;
			var colorParams:ColorParams = new ColorParams(
							lineColor,
							lineWidth,
							lineAlpha,
							shapeColor,
							shapeAlpha,
							ColorParams.COLOR_TYPE_WARM
						);
			...
			if (collisionpoints.length > 0) {
				_form.points = collisionpoints;
			}
			_form.generateShape();
			
			_form.x = _positionLocal.x;
			_form.y = _positionLocal.y;
			_forms.push(_form);
			this.addChild(_form);		
		}
		...
		private function process_Generate_InCircleWithDelta(value:Number, angleFrom:uint = 90, angleTo:uint = 360, correlationIndex:Number = 0.3):void {
			...
			var 	randomangle 	:Number		= random(angleFrom, angleTo)
				,	angle			:Number 	= randomangle * Utils.ANGLE_TO_RADIAN
